...
target_words = {'hello', 'world','restrooms','elevator','restroom','vending','men','women','stair','lab'} 
...
def detectText(frame):
    """Process the given frame to detect specific words and return them as a string."""
    ...
    # Split the text into words and filter based on target words
    detected_words = set(word.lower() for word in text.split())
    matching_words = detected_words.intersection(target_words)
...
while True:
    ...
    # Process the frame to detect specific words
    detected_text = detectText(frameL)
        
    # Print the detected text (for demonstration purposes)
    if detected_text:
